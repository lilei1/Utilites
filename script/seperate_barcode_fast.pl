#!/usr/bin/perl -w
use strict;
##By Li Lei, originate adapted from sicheng Wang's script to do demultiplex for GBS raw data
...
<fastq files or gzipped fastq files: L1_001.fastq.gz L1_002.fastq.gz>
**********************************************************************************
);
...
sub seperate_barcodes
{
	...
	foreach my $file (@files)
	{
		...
		if ($file =~ /\.gz$/){open (IN , "gunzip -c $file |") or die "can't open file $file\n"}
		else{open (IN, $file) or die "can't open file $file\n";}
